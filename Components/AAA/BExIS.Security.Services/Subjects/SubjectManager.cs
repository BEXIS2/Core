...
{      
    ...
    {      
        public SubjectManager()
        {
            ...
            this.AuthenticatorRepo = uow.GetReadOnlyRepository<Authenticator>();
        ...
        public IReadOnlyRepository<Authenticator> AuthenticatorRepo { get; private set; }
        ...
        public bool DeleteGroupById(long id)
        {
            ...
            if (group != null)
            {
                using (IUnitOfWork uow = this.GetUnitOfWork())
                {
                    IRepository<Group> groupRepo = uow.GetRepository<Group>();

                    groupRepo.LoadIfNot(group.Users);
                    group.Users.Clear();
        ...
        {
            if (GroupsRepo.Query(g => g.Id == id).Count() == 1)
            {
                return true;
            }
            else
        ...
        {
            if (GroupsRepo.Query(g => g.Name.ToLower() == groupName.ToLower()).Count() == 1)
            {
                return true;
            }
            else
        ...
        public IQueryable<User> FindUsersInGroup(string groupName, string userNameToMatch)
        {
            return UsersRepo.Query(u => u.Name.Contains(userNameToMatch) && u.Groups.Any(g => g.Name.ToLower() == groupName.ToLower()));
        }
        ...
        {
            ICollection<Group> groups = GroupsRepo.Query(g => g.Id == id).ToArray();
        ...
        {
            ICollection<Group> groups = GroupsRepo.Query(g => g.Name.ToLower() == groupName.ToLower()).ToArray();
        ...
        public IQueryable<Group> GetGroupsFromUser(string userName)
        {
            return GroupsRepo.Query(g => g.Users.Any(u => u.Name.ToLower() == userName.ToLower()));
        }

        public IQueryable<User> GetUsersFromGroup(string groupName)
        {
        ...
        {
            if (GetUsersFromGroup(groupName).Count() > 0)
            {
                return true;
            }
            else
        ...
        {
            if (GetUsersFromGroup(groupName).Where(u => u.Name.ToLower() == userName.ToLower()).Count() == 1)
            {
                return true;
            }
            else
        ...
        public User CreateUser(string userName, long authenticatorId)
        {
            ...
            {
                Name = userName,
                Authenticator = AuthenticatorRepo.Get(authenticatorId),
        ...
        public User CreateUser(string userName, string email, string password, string passwordQuestion, string passwordAnswer)
        {
            throw new NotImplementedException();
        }
        ...
        public bool ExistsUserWithUserNameAndAuthenticatorId(string userName, long authenticatorId)
        {
            if (UsersRepo.Query(u => u.Name.ToLower() == userName.ToLower() && u.Authenticator.Id == authenticatorId).Count() == 1)
            {
                return true;
            }
            else
