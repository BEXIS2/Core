...
namespace BExIS.IO.Transform.Validation.DSValidation
{
    ...
    public class DatastructureMatchCheck : IDataStructureValidation
    {
        ...
        public List<Error> Execute(List<VariableIdentifier> checkList, List<VariableIdentifier> sourceList, string sourceListName)
        {
            ...
            {
                errors.Add(new Error(ErrorType.Datastructure, "Data structure does not fit. The number of variables in the file ( "+ checkList.Count +" ) does not match with the defined number in the data structure ( " + sourceList.Count + " ).", sourceListName));
                return errors;
            }
