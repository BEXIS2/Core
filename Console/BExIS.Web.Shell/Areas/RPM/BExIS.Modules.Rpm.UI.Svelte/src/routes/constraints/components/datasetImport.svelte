<script lang="ts">
	...
	function updateData() {
		...
		if (meta.dataset) dataset = meta.dataset;
		if (meta.datasets != datasets) datasets = meta.datasets;
	}
	...
	function updateStore() {
		if (dataset) {
			...
			if (dataset.varId && dataset.varId > 0) disable = false;
		}
...
{#if datasets && datasets.length > 0}
	<div class="w-9/10 m-5 left-1/20">
		...
		<div class="border-b border-primary-500 pb-1">
			<h3 class="h3 h-9">Import Domain List from Dataset</h3>
		</div>
									...
									{#each dataStructure.columnInfos as columnInfo}
										{#if dataset.varId == columnInfo.id}
											<th class="text-left bg-secondary-300 px-1">
												<RadioItem
													class="clip rounded variant-soft-secondary overflow-hidden w-full shadow-md"
													on:change={() => {}}
										...
										{:else}
											<th class="text-left px-1">
												<RadioItem
													class="clip rounded variant-filled-primary overflow-hidden w-full shadow-md"
													on:change={() => {}}
													bind:group={dataset.varId}
													name={columnInfo.name}
													id={columnInfo.id}
