...
<script lang="ts">
 ...
 <div class = "w-full flex items-center justify-end gap-x-4 pr-5">
 <!--align the upload button to the left 
    <div
      title="Upload new metadata to update the form. This will override all unsaved changes. Current supported formats are .json and .xml."
      class="tooltip tooltip-bottom"
    >-->
      <span
        title="Upload new metadata to update the form. This will override all unsaved changes. Current supported formats are .json and .xml."
        class="tooltip tooltip-bottom cursor-pointer mr-auto pl-2"
      >
        <FileButton
          name="files"
      ...
      <!-- Cancel -->
       <button
        ...
        <button
          ...
          title={disbaleSaveBtn ? "Please fix validation errors before saving." : "Save changes"}
          ...
          on:click={async () => {
            try {
              ...
              notificationStore.showNotification({
                ...
                message: 'Error saving metadata. Please check your input and try again. If the problem persists, contact the support.',
              });
            }
          }}>
          <Fa icon={faSave}/>&nbsp;Save
