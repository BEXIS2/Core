   ...
   <div id="variable-{index}-dataprev " class="flex-col text-left w-52">
    ...
    <div id="variable-{index}-content" class="flex-col space-y-2 w-52">
      ...
      <div class="text-right">
       <!--datatype-->
       {#if datatype}<span title="data type" class="badge variant-filled-primary" >{datatype}</span>{/if}

       <!--unit-->
       {#if unit}<span title="unit"  class="badge variant-filled-secondary" >{unit}</span>{/if}

        <!--variable template-->
       {#if template}<span title="template"  class="badge variant-filled-surface" >{template}</span>{/if}
