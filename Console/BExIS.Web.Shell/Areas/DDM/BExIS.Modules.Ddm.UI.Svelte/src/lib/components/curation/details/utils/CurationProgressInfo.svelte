<script lang="ts">
	...
	import { CurationFilterType } from '$lib/models/Curation';

	export let type: CurationEntryType | undefined = undefined;
	export let expandWritable: Writable<boolean> | undefined = undefined;

	const { statusColorPalette, curation } = curationStore;

	$: progress = type ? $curation?.curationProgressPerType[type] : $curation?.curationProgressTotal;
	const label = `${type ? CurationEntryTypeNames[type] : 'Curation'} Progress`;

	$: totalIssues = progress?.reduce((a, b) => a + b, 0) ?? 0;
	...
	const hasLowOpacity = derived(
		...
		([$statusFilter, $hoveredStatus]) => {
			if (!progress) {
				return CurationEntryTypeNames.map(() => false);
			}
...
<div class="curation-status-progress-card p-2">
	{#if progress}
		...
		<ul class="flex flex-row flex-wrap justify-between gap-x-4 text-sm">
			{#if progress && progress.slice(0, CurationEntryStatus.Closed).some((p) => p > 0)}
				{#each progress as p, index}
					{#if p > 0}
						<li>
							<div
