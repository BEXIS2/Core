<script lang="ts">
	...
	const filter = writable<number | undefined>(undefined);
	...
	<div data-popup={popupId} id={popupId} class="z-50">
		<div class="bg-base-100 card max-w-64 p-3 shadow-lg">
			<label class="label flex flex-col text-sm normal-case">
				<span>Only show datasets with curation status:</span>
				<select
					bind:value={$filter}
				...
				>
					...
					{#each CurationStatusLabels as cs, index}
						<option value={index + 2}>{cs.name}</option>
					{/each}
				</select>
			</label>
		</div>
