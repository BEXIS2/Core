...
namespace BExIS.Ddm.Providers.OpenSearchProvider
{
    public class OpenSearchIndexer : IDisposable
    {
        ...
        public void EnsureMappingMatches(string indexName, string mappingJsonPath)
        {
            ...
            {
                Console.WriteLine("⚠️ Mapping unterscheidet sich! Bitte manuell prüfen oder Reindex einleiten.");
        ...
        public SearchResult Search(QueryContainer query, List<XmlNode> headerItemXmlNodeList)
        {
            int n = 0;
            ...
            foreach (var hit in searchResponse.Hits)
            {
                ...
                // Standardfelder wie in deinem Originalcode
                ...
                foreach (XmlNode ade in headerItemXmlNodeList)
                {
                    ...
                    if (fieldType.Equals("facet_field", StringComparison.OrdinalIgnoreCase))
                    {
                        luceneName = "facet_" + luceneName;
                    }
                    else if (fieldType.Equals("category_field", StringComparison.OrdinalIgnoreCase))
                    {
                        luceneName = "category_" + luceneName;
                    }
                    else if (fieldType.Equals("property_field", StringComparison.OrdinalIgnoreCase))
                    {
                        luceneName = "property_" + luceneName;
                    }

                    if (source.TryGetValue(luceneName, out var fieldValues))
                    {
        ...
        )
        {
            ...
            foreach (var f in facets)
            {
                ...
                foreach (var bucket in buckets)
                {
                    ...
                    if (_configXML != null &&
                        _configXML.GetElementsByTagName("field") is XmlNodeList fieldNodes)
                    {
                        // Top‑Hits‑Dokumente des Buckets
                        var docs = bucket.TopHits("docs")?.Documents<dynamic>() ?? Enumerable.Empty<dynamic>();

                        foreach (XmlElement item in fieldNodes)
                        {
                            var primType = item.GetAttribute("primitive_type").ToLower();
                            var displayName = item.GetAttribute("display_name").ToLower();

                            if (!primType.Contains("date") &&
                                !primType.Contains("string") &&
                                displayName.Contains(c.DisplayName.ToLower()))
                            {
                                foreach (var doc in docs)
                                {
                                        ...
                                        dict.TryGetValue(facetField, out var raw))
                                    {
                                        ...
                                        {
                                            if (!c.Childrens.Any(x => x.Name == val))
                                            {
                                                c.Childrens.Add(new Facet
                                                {
                                                    Name = val,
                                                    Text = val,
                                                    Value = val
                    ...
                    c.Childrens.Add(cc);   // analog zum Original immer hinzufügen
                }
